...
<body>
    ...
    <?php
    $tikid = $_GET["id"];
    $query16 = "SELECT * FROM `tickets` WHERE `TICKET_ID`='$tikid'";
    $result16 = mysqli_fetch_assoc(mysqli_query($con, $query16));
    if(isset($_POST["submit"])){
        $title = $_POST["title"];
        $price = $_POST["price"];
        $query15 = "UPDATE `tickets` SET `DESC_`='$title',`PRICE`='$price' WHERE `TICKET_ID`='$tikid'";
        ...
        <h3 style=" text-align:center; margin-bottom:0%; color:black;">Ticket Updated</h3>
        <?php
    }
    ?>

    <h1 style=" text-align:center; font-size:60px; margin:20px; color:black;"><i>Edit Ticket</i></h1>

    <form method="post" style=" margin:50px 150px 50px 150px;">
        <label for="title">Title</label>
        <input type="text" name="title" id="title" value="<?= $result16["DESC_"]?>" required/>

        <hr style=" opacity: 0;"/>

        <label for="price">Price</label>
        <input type="number" name="price" id="price" value="<?= $result16["PRICE"]?>" required/>
